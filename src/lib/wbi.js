...
function encWbi(params, img_key, sub_key) {
  ...
  Object.keys(params).sort().forEach((key) => {
    query.push(`${encodeURIComponent(key)}=${encodeURIComponent(// 过滤 value 中的 "!'()*" 字符
...
async function getWbiKeys() {
  const resp = await axios({
    url: 'https://api.bilibili.com/x/web-interface/nav', method: 'get', responseType: 'json'
  }), json_content = resp.data, img_url = json_content.data.wbi_img.img_url, sub_url = json_content.data.wbi_img.sub_url
...
const getWbiQuery = (queryInfo) => {
  return getWbiKeys().then((wbi_keys) => {
    const query = encWbi({
      mid: queryInfo.mid
